...
//
// This is deliberately identical to pxr GfMatrix4d (matrix4d.h:56) so hydra/
// can copy element-for-element with no transpose, and so tests can assert exact
...
struct mat4
{
  ...
  // treats v as (x,y,z,0): no translation, no divide.  ==
  // GfMatrix4d::TransformDir
  [[nodiscard]] vec3 transform_dir(const vec3 &v) const
  {
...
// random matrices and to 1.8e-15 against GfMatrix4d::GetInverse (step 2, step
// 9).
...
// the Hydra delegate receives matrices already built and must never construct
// its own.
// ---------------------------------------------------------------------------
...
// View-to-NDC, symmetric window. Identical to
// GfFrustum::ComputeProjectionMatrix for GfFrustum::Perspective
// (frustum.cpp:560-574). Leaves m[3][3] == 0, which is what camera's
// orthographic test keys off.
...
// View-to-NDC, symmetric window. Identical to
// GfFrustum::ComputeProjectionMatrix for GfFrustum::Orthographic
// (frustum.cpp:552-558). Leaves m[3][3] == 1.
...
// Integer pixel rect. y-DOWN, max INCLUSIVE - the same semantics as pxr
// GfRect2i and therefore as HdRenderPassState::GetFraming().dataWindow
// (hydra-spec §9).
struct rect2i
{
