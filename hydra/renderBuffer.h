...
public:
  ...
  bool Allocate(GfVec3i const &dims, HdFormat format, bool multiSampled) override
  {
    ...
    return _buf.allocate(dims[0], dims[1], static_cast<buffer_format>(int(format)), multiSampled);
  }

  [[nodiscard]] unsigned int GetWidth() const override { return _buf.width(); }
  [[nodiscard]] unsigned int GetHeight() const override { return _buf.height(); }
  [[nodiscard]] unsigned int GetDepth() const override { return 1; }
  [[nodiscard]] HdFormat GetFormat() const override { return static_cast<HdFormat>(int(_buf.format())); }
  [[nodiscard]] bool IsMultiSampled() const override { return _buf.is_multisampled(); }

  void *Map() override { return _buf.map(); }
  void Unmap() override { _buf.unmap(); }
  [[nodiscard]] bool IsMapped() const override { return _buf.is_mapped(); }
  void Resolve() override { _buf.resolve(); }
  [[nodiscard]] bool IsConverged() const override { return _buf.is_converged(); }
